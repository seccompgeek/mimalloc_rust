...
unsafe impl GlobalAlloc for MetaSafeAlloc {
    ...
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        if layout.align() < MIN_ALIGN {
            return ffi::mi_malloc(layout.size()) as *mut u8;
        }
        ffi::mi_malloc(layout.size()) as *mut u8
    }

    #[inline]
    unsafe fn alloc_zeroed(&self, layout: Layout) -> *mut u8 {
        let ptr = if layout.align() < MIN_ALIGN {
            ffi::mi_malloc(layout.size())
        } else {
            ffi::mi_malloc(layout.size())
        };
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: Layout, new_size: usize) -> *mut u8 {
        let _align = if layout.align() < MIN_ALIGN { MIN_ALIGN } else {layout.align()};
